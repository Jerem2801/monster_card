...
import { updateLegendaryMonster2 } from '@/component/monster-card/legendary/LegendaryMechanics';
...
export function useMonsterCard(monster){
  ...
  const setBloodiedStatus = useCallback(bloodied => {
    setStatus('bloodied', bloodied);
  }, [setStatus]);

  // Met à jour le statut dead/deadly
  const setDeadStatus = useCallback((newHp, legendary) => {
  ...
  const updateLegendaryMonster = useCallback(newHp => {
  ...
  const handleNewHP = useCallback(newHp => {
    if (!localMonster) return;
  ...
  const addCustomStatus = useCallback(status => {
