...
export default function StatusPanel({status}){
    ...
    return (
        <div className="flex items-center gap-1">
            {status.map(status => (
                <Popover key={status.id} content={getPopoverContent(status.id)} trigger="hover" placement="top">
                    <Image key={status.id} src={status.path} alt={status.label} width={24} height={24} />
                </Popover>
            ))}
        </div>
    );
