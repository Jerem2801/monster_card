        ...
        {statusOrder.map(status => {
          ...
          return (
            <label
              ...
              className={`flex items-center gap-3 cursor-pointer rounded-md border px-3 py-2 transition
                ${isChecked ? 'bg-indigo-100 border-indigo-400' : 'border-gray-300 hover:bg-gray-100'}`}
            >
              <input
                type="checkbox"
                id={`status-${status.id}`}
              ...
              <span
                className={`inline-flex h-5 w-5 flex-shrink-0 items-center justify-center rounded border
                  ${isChecked ? 'border-indigo-600 bg-indigo-600' : 'border-gray-400 bg-white'}`}
              >
                {isChecked && (
                  <svg
