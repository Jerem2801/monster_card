...
import ResultDisplayDialogBox from '@/component/fight/ui/resultDisplay/ResultDisplayDialogBox';
    ...
    function updateMonsterHp(id, newHp) {
        setMonsters(prev =>
            prev.map(m => {
                ...
                let newStatus = [...m.status].filter(s => s.id !== BLOODIED.id && s.id !== DEAD.id);
    ...
    function updateMonsterStatus(id, newStatusList) {
        setMonsters(prev =>
            prev.map(m => {
                ...
                const autoStatusIds = [BLOODIED.id, DEAD.id];
